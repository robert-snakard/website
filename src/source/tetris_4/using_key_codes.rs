...
pub fn run() {
    ...
    EventListener::new(&window().unwrap(), "keydown", move |event| {
        move_piece(event.dyn_ref::<KeyboardEvent>().unwrap());
    }).forget();
...
fn move_piece(event: &KeyboardEvent) {
    match event.key_code() {
        37 => console::log_1(&JsValue::from("move left")),
        39 => console::log_1(&JsValue::from("move right")),
        _ => {},
    }
}
